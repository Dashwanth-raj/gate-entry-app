...
const Dialog = ({ open, onOpenChange, children }) => (
  <div style={{ display: open ? 'block' : 'none' }}>
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => onOpenChange(false)}></div>
    <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl p-8 z-50 sm:max-w-[480px] w-[90vw] max-h-[90vh] overflow-y-auto transform transition-all duration-300 ease-out">
      {children}
    </div>
  </div>
);
...
function App() {
  ...
  const [isCameraPermissionModalOpen, setIsCameraPermissionModalOpen] = useState(false); // New state for camera permission modal
  ...
  // Handles starting/stopping camera and scanning license plate
  const handleScanLicensePlate = useCallback(async () => {
    if (isCameraActive) {
      // If camera is active, capture a frame and perform OCR
  ...
  // Downloads vehicle history as an XML file
  const downloadHistoryAsXml = () => {
    if (vehicleHistory.length === 0) {
      toast.error("No history to download.");
      return;
    }

    const xmlDoc = document.implementation.createDocument(null, "VehicleHistory", null);
    const historyElement = xmlDoc.documentElement;

    vehicleHistory.forEach(entry => {
      const vehicleElement = xmlDoc.createElement("VehicleLog");
      vehicleElement.setAttribute("id", entry.id);

      const licensePlateElement = xmlDoc.createElement("LicensePlate");
      licensePlateElement.textContent = entry.licensePlate;
      vehicleElement.appendChild(licensePlateElement);

      const numPeopleElement = xmlDoc.createElement("NumPeople");
      numPeopleElement.textContent = entry.numPeople.toString();
      vehicleElement.appendChild(numPeopleElement);

      const purposeElement = xmlDoc.createElement("Purpose");
      purposeElement.textContent = entry.purpose;
      vehicleElement.appendChild(purposeElement);

      const approvedByElement = xmlDoc.createElement("ApprovedBy");
      approvedByElement.textContent = entry.approvedBy;
      vehicleElement.appendChild(approvedByElement);

      const entryTimeElement = xmlDoc.createElement("EntryTime");
      entryTimeElement.textContent = entry.entryTime.toISOString();
      vehicleElement.appendChild(entryTimeElement);

      const exitTimeElement = xmlDoc.createElement("ExitTime");
      exitTimeElement.textContent = entry.exitTime ? entry.exitTime.toISOString() : "N/A";
      vehicleElement.appendChild(exitTimeElement);

      const statusElement = xmlDoc.createElement("Status");
      statusElement.textContent = entry.status;
      vehicleElement.appendChild(statusElement);

      const securityPersonnelIdElement = xmlDoc.createElement("SecurityPersonnelId");
      securityPersonnelIdElement.textContent = entry.securityPersonnelId;
      vehicleElement.appendChild(securityPersonnelIdElement);

      historyElement.appendChild(vehicleElement);
    });
    ...
    a.href = url;
    a.download = `vehicle_history_${new Date().toISOString().split('T')[0]}.xml`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    toast.success("Vehicle history downloaded as XML!");
  };
        ...
        {isCameraActive && (
          <div className="mb-8 p-6 border border-blue-300 rounded-xl bg-blue-50 shadow-md">
            ...
            <div className="flex justify-center gap-6 mt-6">
              <Button
                onClick={handleScanLicensePlate}
                disabled={isScanning}
                className={`
        ...
        <div className="mb-8 p-6 border border-blue-200 rounded-xl bg-blue-50 shadow-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <Button
                  onClick={handleScanLicensePlate}
                  size="icon"
                  className={`shrink-0 w-12 h-12 text-xl ${isCameraActive ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-600 hover:bg-gray-700'}`}
                  title={isCameraActive ? "Scan License Plate" : "Start Camera"}
                  disabled={isScanning}
                >
                  <i className={isCameraActive ? "fas fa-camera" : "fas fa-video"}></i>
                  <span className="sr-only">{isCameraActive ? "Scan License Plate" : "Start Camera"}</span>
                </Button>
              </div>
            ...
            <div className="md:col-span-2"> {/* Purpose takes full width on larger screens */}
              ...
              <Button
                onClick={handleSuggestPurposeDetails}
                disabled={isGeneratingPurpose}
                className="mt-4 w-full bg-purple-600 text-white hover:bg-purple-700 focus:ring-purple-500 text-base shadow-md"
                title="Get AI-powered purpose suggestions"
              >
                {isGeneratingPurpose ? (
        ...
        <div className="p-6 border border-gray-300 rounded-xl bg-gray-50 shadow-md">
          <h2 className="text-2xl font-bold text-gray-700 mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between">
            ...
            <div className="flex space-x-2 mt-3 sm:mt-0">
              <Button
                onClick={downloadHistoryAsXml}
                className="bg-gray-600 text-white hover:bg-gray-700 shadow-sm text-sm"
                size="sm"
                title="Download History as XML"
              >
                <i className="fas fa-file-download mr-1"></i> XML
              </Button>
              <Button
